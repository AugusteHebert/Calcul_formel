...
\begin{Prop}
Soient $p\in \mathbb{P}_{\geq 3}$ , $\alpha\in \N^*$ et $a\in \Z$.  On écrit $a=p^k a'$, où $k\in \llbracket 0,\alpha-1\rrbracket$ et $a'\in \Z$ vérifie $a'\wedge p=1$. Alors $a$ est un carré modulo $p^\alpha$ si et seulement les conditions suivantes sont vérifiées : \begin{equation}
\item 
\end{equation}

 Alors $x$ est un carré modulo $p^\alpha$ si et seulement si $k$ est pair et $a'$ est un carré modulo $p$.


\end{Prop}

\begin{proof}
 On le démontre par récurrence sur $k$. On suppose que pour tout $\tilde{\alpha}\in \llbracket 0,\alpha-1\rrbracket$, pour tout $\tilde{k}\in \llbracket 0,\tilde{\alpha}-1\rrbracket$, pour tout $\tilde{a}\in \llbracket 0,p^{\tilde{\alpha}-\tilde{k}}-1\rrbracket$ tel que $\tilde{a}\wedge p=1$, $p^{\tilde{k}}\tilde{a}$ admet une racine carrée modulo $p^{\tilde{\alpha}}$, alors $\tilde{k}$ est pair et $\tilde{a}$ est un carré modulo $p$.    Si $k=0$, c'est une conséquence du lemme~\ref{lemExistence_racine_modulo_pAlpha_inversible}. Supposons que $k\geq 1$. Supposons qu'il existe une racine $\gamma$ de $a$ modulo $p^\alpha$. Alors il existe $\ell\in \Z$ tel que $\gamma^2=a+\ell p^\alpha=p^k a'+\ell p^\alpha$. On en déduit que $p$ divise $\gamma^2$. D'après le lemme d'Euclide, $p$ divise  $\gamma$. On peut donc écrire $\gamma=p \gamma'$, où $\gamma'\in \Z$. Alors $p^2\gamma'^2=p^k a'+\ell p^\alpha$. On a donc $p\gamma'-\ell p^{\alpha-1}=p^{k-1} a'$. Comme $\alpha>k>1$, on en déduit que $p$ divise $p^{k-1}a'$, donc $p$ divise $p^{k-1}$, d'après le lemme de Gauss. On a donc $k\geq 2$. Alors \[\gamma'^2=p^{k-2} a'+\ell p^{\alpha-2}.\] D'après l'hypothèse de récurrence, on en déduit que $k-2$ est pair et que $a'$ est un carré modulo $p$.

Réciproquement, supposons que $k$ est pair et que $a'$ est un carré modulo $p$. D'après le lemme~\ref{lemExistence_racine_modulo_pAlpha_inversible}, il existe $\gamma',\ell'\in \Z$ tel que $\gamma'^2=a'+\ell'p^{\alpha-k}$. Alors $(p^{k/2}\gamma')^2=p^k(a'+\ell'p^{\alpha-k})=p^k\alpha+\ell' p^\alpha$. Donc $a$ est bien un carré modulo $p^\alpha$, ce qui démontre (1). 
\end{proof}


\begin{Prop}
Soient $p\in \mathbb{P}_{\geq 3}$ , $\alpha\in \N^*$ et $a\in \llbracket 0,p^\alpha-1\rrbracket$. On écrit $a=p^k a'$, où $k\in \llbracket 0,\alpha-1\rrbracket$ et $a'\in \llbracket 0,p^{\alpha-k}-1\rrbracket$ vérifie $a'\wedge p=1$.
