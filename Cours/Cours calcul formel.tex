...
\item Soit   $r\in m+n\Z$. Alors    $\big(\frac{m}{n}\big)=\big(\frac{r}{n}\big)$. C'est en particulier vrai si $r$ est le reste dans la division euclidienne de $m$ par $n$.
...
\begin{lemme}\label{l_car_ss_gpe}
Soient $G$ un groupe et $H$ un sous-ensemble fini de $G$ contenant $1$ et stable par multiplication. Alors $H$ est un sous-groupe de $G$. 
\end{lemme}

\begin{proof}
Le seul point à montrer est que $H$ est stable par passage à l'inverse. Pour $g\in G$, on définit $m_g:H\rightarrow H$  par $m_g(h)=gh$, pour $h\in H$. Soit $g\in H$. Alors $m_g$ est bijective, d'inverse $m_{g^{-1}}$. En particulier, $m_g$ est surjective donc il existe $h\in H$ tel que $m_g(h)=1$. On a donc $gh=1$, donc $h=g^{-1}\in H$ et $H$ est un sous-groupe de $G$. 
\end{proof}
...
\item si $n$ est composé, $\{\overline{a}\in (\Z/n\Z)^\times \mid  \overline{a^{(n-1)/2}}=\overline{\left(\frac{a}{n}\right)}\}$ a au plus $\varphi(n)/2$ éléments.
\end{enumerate}
\end{thm}
...
(2) Soit $H=\{\overline{a}\in \Z/n\Z\mid \overline{a}^{(n-1)/2}\equiv \left( \frac{a}{n}\right)\}$. Alors $\overline{1}\in H$. Soient $\overline{a},\overline{b}\in H$. Alors $\overline{ab}^{(n-1)/2}=\overline{a}^{(n-1)/2}\overline{b}^{(n-1)/2}=\overline{\frac{a}{n}}\overline{\frac{b}{n}}=\overline{\frac{ab}{n}}$. On en déduit que $H$ est stable par multiplication et donc que $H$ est un sous-groupe de $(\Z/n\Z)^\times$, d'après le lemme~\ref{l_car_ss_gpe}. D'après (1),  $H$ est un sous groupe strict de $(\Z/n\Z)^\times$. D'après  le théorème de Lagrange, $|H|$ divise $|(\Z/n\Z)^\times|=\varphi(n)$, d'où le résultat.
\end{proof}
...
Montrons que trouver la clé secrète $d$ uniquement à  l'aide de la clé publique $(n,e)$ est calculatoirement  équivalent à  factoriser $n$. Plus précisément on montre que si on sait factoriser $n$, on sait calculer «~rapidement~» (en utilisant des algorithmes de complexité polynomiale) $d$ et réciproquement (on obtient une méthode probabiliste qui utilise des algorithmes de complexité polynomiale).

Si on sait factoriser $n=pq$, on peut alors calculer $\varphi=(p-1)(q-1)$; puis $d$ tel que $1<d<\varphi$ et $ed \equiv 1 [\varphi]$.


Réciproquement, on suppose que l'on connaît $n,e$ et $d$. On va montrer comment factoriser $n$.



Comme $ed \equiv 1 [\varphi]$, il existe $k \in \mathbb{Z}$ tel que $1=ed + k \varphi$, donc si $a \in (\mathbb{Z}/n\mathbb{Z})^{\times}  $, on a $a^{ed-1}\equiv 1 [n]$. Décomposons à  présent $ed-1$ sous la forme $ed-1=2^s t$ où $s \in \mathbb{N}$ et $t$ impair.


Pour cela, on se base sur le lemme suivant. Si $x\in \Z$ et $\ell\in \N$, on note $x[\ell]$ l'image de $x$ dans $\Z/\ell\Z$. On note $f:\Z/n\Z\rightarrow \Z/p\Z\times \Z/q\Z$ l'isomorphisme chinois, défini par $f(x[n])=(x[p],x[q])$ pour $x[n]\in \Z/n\Z$. 

\begin{lemme}\label{l_fact_n}
Soit $b\in \Z$. On suppose que $b^2\equiv 1[n]$ et $b\not \equiv \pm 1[n]$. Alors $(b-1)\wedge n$ est un diviseur non trivial de $n$. 
\end{lemme}

\begin{proof}
Dans $\Z/n\Z$, on a $\underbrace{(\overline{b}-\overline{1})}_{\neq \overline{0}}\underbrace{(\overline{b}+\overline{1})}_{\neq \overline{0}}=\overline{b}^2-\overline{1}=\overline{0}$. On en déduit que $\overline{b}-\overline{1}$ et non nul et non-inversible dans $\Z/n\Z$, donc que $(b-1)\wedge n$ est un diviseur non-trivial de $n$. 
\end{proof}


L'idée est alors de trouver $a\in \Z/n\Z$ tel que :\begin{enumerate}
\item $a^t\neq 1[n]$

\item il existe $j\in \llbracket 1,s\rrbracket$ tel que $a^{2^{j-1}t}\neq \pm 1[n]$ et $a^{2^jt}=1[n]$.
\end{enumerate}
On peut alors appliquer le lemme~\ref{l_fact_n} à $a^{2^{j-1}t}$, ce qui permet de factoriser $n$. Il suffit donc de voir qu'il est possible de trouver $a$ satisfaisant les deux propriétés précédentes. On va montrer qu'au moins $3/8$ éléments de $(\Z/n\Z)^\times$ satisfont cette propriété (voir \eqref{e_suffisament}). Il suffit  donc de choisir des éléments de $(\Z/n\Z)^\times$ au hasard et de tester s'ils satisfont cette propriété. La suite de la partie est dédiée à la preuve de \eqref{e_suffisament}.




On pose $G=(\Z/n\Z)^\times$.

\begin{lemme}\label{l_X_G}
Soit $X=\{a\in G\mid a^t\neq \overline{1}\}$. Alors $|X|\geq \frac{3}{4}\varphi$.
\end{lemme}

\begin{proof}
Soit $\psi:G\rightarrow G$ défini par $\psi(a)=a^t$, pour $a\in G$.   Soit $H=\ker(\psi)$. On a $|G|=|\ker(\psi)||\mathrm{Im}(\psi)|=|H||\mathrm{Im}(\psi)|$, donc $|G/H|=\mathrm{Im}(\psi)$. Comme $X=G\setminus H$, on a \[X=|G|-|H|=(1-|H|/|G|)|G|=(1-1/|\mathrm{Im}(\psi)|)|G|.\]  


Si $y\in \{(\pm 1[p],\pm 1[q])\}\subset \Z/p\Z\times \Z/q\Z$, $y^t=y$, car $t$ est impair, donc si $x\in \{f^{-1}(\pm1[p],1[q])\}\subset G$, on a $x^t=x$, donc $x\in \mathrm{Im}(\psi)$. On en déduit que $|\mathrm{Im}(\psi)|\geq 4$ et donc $|X|\geq 3/4 |X|$. 
\end{proof}

Pour $a\in G$, on a $a^\varphi=\overline{1}$, donc $a^{2^st}=\overline{1}$. Si $j\in \llbracket 1,s\rrbracket$, on pose \[X_j=\{a\in X\mid a^{2^{j-1}t}\neq \overline{1}, a^{2^j t}=\overline{1}\}.\] On a \[X=\bigsqcup_{j=1}^s X_j.\]
...
On écrit $p-1=2^{s_p} t_p$ et $q-1=2^{s_q}t_q$, où $s_p,s_q\in \N$ et $t_p,t_q\in \N$ sont impairs. Quitte à échanger $p$ et $q$, on peut supposer que $s_p\leq s_q$. 


\begin{lemme}\label{l_ineq_j}
Soit $j\in \llbracket 1,s\rrbracket$ tel que $X_j\neq \emptyset$. Alors $j\leq s_q$. 
\end{lemme}

\begin{proof}
Soit  $a\in X_j$. Soit $b=a^t\in G$.  On a $b^{2^{j-1}}\neq \overline{1}$, donc $f(b^{2^{j-1}})\neq (1[p],1[q])$. Autrement dit,  $b^{2^{j-1}}[p]\neq 1[p]$ ou $b^{2^{j-1}}[q]\neq 1[q]$. On a donc $b[p]$ d'ordre $2^j$ ou $b[q]$ d'ordre $2^j$. On en déduit que $2^j| (p-1)$ ou $2^j|(q-1)$, d'après le théorème de Lagrange. 
\end{proof}

\begin{lemme}\label{l_Y_X}
Soit $Y= \{a\in X\mid \exists j\in \llbracket 1,s\rrbracket, a^{2^{j-1}t}\neq \pm \overline{1}, a^{2^jt}=\overline{1}\}.$
On a $|Y|\geq \frac{1}{2}|X|$.
\end{lemme} 

\begin{proof}
 Si $j\in \llbracket 1,s\rrbracket$, on pose $Y_j=Y\cap Y_j$. On a alors $Y_j=\{a\in X\mid a^{2^{j-1}t}\neq  \pm \overline{-1}, a^{2^j t}=\overline{1}\}$.  Soit $j\in \llbracket 1,s\rrbracket$ tel que $X_j\neq \emptyset$. Comme $((\Z/q\Z)^\times,.)$ est cyclique, on peut choisir un générateur $\alpha$ de $(\Z/q\Z)^\times$. Soit $\beta=\alpha^{2^{s_q-j}t_q}\in (\Z/q\Z)^\times$ (par le lemme~\ref{l_ineq_j}, $s_q-j\geq 0$). Alors $\beta$ est d'ordre $2^j$. On a donc $(\beta^{2^{j-1}})^2=1[q]$ et $\beta^{2^{j-1}}\neq 1[q]$. Comme $\Z/q\Z$ est un corps, on en déduit que $\beta^{2^{j-1}}=-1[q]$ (par le corollaire~\ref{corRacines_carrées_moins_un}). 

 Montrons que $|Y_j|\geq |X_j|/2$. Si $X_j=Y_j$, c'est clair. Supposons que $X_j\neq Y_j$. Soit $\eta_j=f^{-1}(1[p],\beta)\in G$. Soit $a\in X_j\setminus Y_j$. On a $a^{2^{j-1}t}=-1[n]$ et  $(a\eta_j)^{2^jt}=a^{2^jt}=1[n]$. On a également $(a\eta_j)^{2^{j-1}t}=a^{2^{j-1}t} (\eta_j)^{2^{j-1}t}$. On a  \[f(\eta_j^{2^{j-1}t})=(1[p],\beta^{2^{j-1}t})=(1[p],(-1)^t[q])=(1[p],-1[q]),\] donc $\eta_j^{2^{j-1}t}=f^{-1}(1[p],-1[q])$. On a donc $f((a\eta_j)^{2^{j-1}t})=(-1[p],1[q])\neq f(-1[n])$, donc $a\eta_j\in Y_j$. On a donc montré : \[\eta_j \cdot (X_j\setminus Y_j)\subset Y_j,\] donc $|Y_j|\geq 1/2 |X_j|$.  
 
 Comme $Y_j=\bigsqcup_{j=1}^s Y_j$ et $X=\bigsqcup_{j=1}^s X_j$, on en déduit que $|Y|\geq 1/2 |X|$. 
\end{proof}


En combinant les lemmes~\ref{l_X_G} et \ref{l_Y_X}, on en déduit : \begin{equation}\label{e_suffisament}|\{a\in (\Z/n\Z)^\times \mid a^t\neq 1[n], \exists j\in \llbracket 1,s\rrbracket, a^{2^{j-1}t}\neq \pm 1[n], a^{2^j t}=1[n]\}|\geq \frac{3}{8}|(\Z/n\Z)^\times.
\end{equation}
...
\chapter{Recherche de racines carrées dans $\Z/n\Z$}

Soit $n\in \N^*$. On se place dans $\Z/n\Z$. Dans cette partie, on s'intéresse au problème suivant. Soit $\overline{a}\in \Z/n\Z$.\begin{enumerate}
\item $\overline{a}$ est-il un carré dans $\Z/n\Z$ ?

\item Si $\overline{a}$ est un carré dans $\Z/n\Z$, déterminer les racines de $\overline{a}$ dans $\Z/n\Z$. 
\end{enumerate}

Par simplicité, on se restreint au cas où $n$ est un produit de nombres impairs, congrus à $3$ modulo $4$.

Supposons que l'on sache factoriser $n$.  On écrit $n=\prod_{i=1}^k p_i^{\alpha_i}$, où $k\in \N$,  $p_i\in \mathbb{P}$ pour tout $i\in \llbracket 1,k\rrbracket$, $\alpha_i\in \N^*$ pour tout $i\in \llbracket 1,k\rrbracket$ et les $p_i$ sont tous distincts.  Ce problème se réduit, grâce au théorème chinois, à la recherche des racines carrées de $a$ modulo $p_i^{\alpha_i}$, pour tout $i\in \llbracket 1,k\rrbracket$. 

Soient $a\in \Z$ et $p\in \mathbb{P}_{\geq 3}$. Nous allons commencer par décrire un algorithme qui permet de trouver une racine carrée de $a$ modulo $p$. Nous verrons ensuite comment en déduire une racine carrée de $a$ modulo $p^\alpha$, pour tout $\alpha\in \N^*$.

Cette partie se base sur \cite{demazure2008cours}. 

\section{Recherche de racines carrées dans $\Z/p\Z$, pour $p\equiv 3[4]$}

Soient $p\in \mathbb{P}$ tels que $p\equiv 3[4]$ et $a\in \Z$. On se place dans $\Z/p\Z$. On recherche une racine carrée de  $\overline{a}$.     Alors $(\overline{a}^{(p+1)/4})^2=\overline{a}.\overline{a}^{(p-1)/2}=\overline{a}.\left(\frac{a}{p}\right)$. Ainsi, si $\overline{a}$ est un carré, $\left(\frac{a}{p}\right)=1$ et $\overline{a}^{(p+1)/4}$ est une racine de $\overline{a}$.


\begin{ex}
On a $43\equiv 3[4]$ et après calcul, $\left(\frac{35}{43}\right)=1$. On en déduit que $\overline{35}^{11}$ est une racine carrée de $\overline{35}$ dans $\Z/43\Z$. On a $11=2^3+2+1$, donc \[\overline{35}^{11}=\overline{35}^8.\overline{35}^2.\overline{35}=((\overline{35}^2)^2)^2.\overline{35}^2.\overline{35}=(((\overline{-8})^2)^2)^2.(\overline{-8})^2.\overline{-8}.\]

On a $\overline{8}^2=\overline{64}=\overline{21}$, $\overline{21}^2=\overline{441}=\overline{11}$ et $\overline{11}^2=\overline{121}=\overline{-8}$, donc $(-8)^{11}=\overline{-8}.\overline{21}.\overline{-8}=\overline{21}^2=\overline{11}$. On obtient que $\overline{11}$ est une racine carrée de $\overline{35}$ (et on a bien $\overline{11}^2=\overline{121}=\overline{86}+\overline{35}$).
\end{ex}
...
On suppose maintenant $p\equiv 1[4]$. On écrit $p-1=2^{r+1} t$, où $r\in \N$ et $t$ impair. On a alors $\left(\frac{a}{p}\right)=\overline{a}^{2^rt}$. On suppose que l'on connait $\overline{z}\in \Z/p\Z$  tel que $\overline{z}^{2^r}=\overline{-1}$. La méthode suivante, due à Shanks, permet de trouver une racine carrée de $\overline{a}$. 
...
\begin{tabular}{ll}
\textbf{Algorithme :} & Racine($\overline{a}$)\\
& $b,x,y\leftarrow \overline{a}^t,\overline{a},z$\\
& pour $j$ allant de $r$ à 1:\\
& \ \ \ {\rm |} si $b^{2^{j-1}}=\overline{1}$:\\
&  \ \ \   \ \ \ {\rm |}$y\leftarrow y^2$\\
&  \ \ \ {\rm |} sinon:\\
&  \ \ \  \ \ \ {\rm |}$b,x,y\leftarrow by^2,xy,y^2$\\
& Renvoyer $x$.
\end{tabular}

\begin{Prop}
L'algorithme ci-dessus renvoie une racine carrée de $\overline{a}$.
\end{Prop}

\begin{proof}
Pour $j\in \llbracket 1,r\rrbracket$, notons $b_j,x_j, y_j$ les valeurs de $b,x,y$ avant l'étape $j$ et on note $b_{0},x_{0},y_{0}$ les valeurs de $b,x,y$ après l'étape $j=1$.

Montrons par récurrence descendante que \begin{equation}\label{eqInvariant_boucle_shanks}
 \overline{a} b_j=(x_j)^2,\ (y_j)^{2^j}=\overline{-1},\ (b_j)^{2^j}=\overline{1},
 \end{equation} pour $j\in \llbracket 0,r\rrbracket$.

On a $\overline{a}b_r=\overline{a}^{t+1}=(\overline{a}^{(t+1)/2})^2$, $(y_r)^{2^r}=z^{2^r}=\overline{-1}$, $b^{2^r}=\overline{a}^{2^r t}=\overline{a}^{(p-1)/2}=\overline{\left(\frac{a}{p}\right)}=\overline{1}$. Soit $j\in \llbracket 0,r-1\rrbracket$. On suppose que \eqref{eqInvariant_boucle_shanks} est satisfaite pour $j+1$. 

Si $(b_{j+1})^{2^{j}}=\overline{1}$, alors $b_{j+1}=b_j$ et $x_{j+1}=x_j$, on a donc $ab_{j}=(x_{j})^2$ . Alors $(y_{j})^{2^j}=((y_{j+1})^2)^{2^j}=(y_{j+1})^{2^{j+1}}=\overline{-1}$. On a aussi $(b_{j})^{2^{j}}=(b_{j+1})^{2^j}=\overline{1}$, par hypothèse, donc \eqref{eqInvariant_boucle_shanks} est vérifiée pour $j$.

Supposons que  $(b_{j+1})^{2^{j}}\neq \overline{1}$. Alors $(b_{j+1})^{2^{j+1}}=((b_{j+1})^{2^j})^2=\overline{1}$, donc $(b_{j+1})^{2^j}=\overline{-1}$, d'après le corollaire~\ref{corRacines_carrées_moins_un}.

On a $\overline{a}b_j=\overline{a}b_{j+1} (y_{j+1})^2=(x_{j+1})^2(y_{j+1})^2=(x_j)^2$. On a $(y_{j})^{2^j}=((y_{j+1})^2)^{2^j}=(y_{j+1})^{2^{j+1}}=\overline{-1}$. Enfin, $(b_j)^{2^j}=(b_{j+1}(y_{j+1})^2)^{2^j}=(b_{j+1})^{2^j} (y_{j+1})^{2^{j+1}}=\overline{(-1)}\overline{(-1)}=\overline{1}$ et donc \eqref{eqInvariant_boucle_shanks} est satisfaite pour $j$. On en déduit que \eqref{eqInvariant_boucle_shanks} est vraie pour tout $j\in \llbracket 0,r\rrbracket$. 

En particulier, \eqref{eqInvariant_boucle_shanks} est satisfaite pour $j=0$. On a donc $(b_0)^{2^0}=b_0=\overline{1}$ et  \[\overline{a} b_0=\overline{a} b_0=\overline{a}=(x_0)^2.\]
\end{proof}

On a supposé que l'on connaissait $\overline{z}\in\Z/p\Z$ tel que $\overline{z}^r=\overline{-1}$. Il reste à trouver un tel $\overline{z}$. Soit $\overline{x}\in \Z/p\Z^*$. Supposons que $x$ n'est pas un résidu quadratique. Alors $\overline{x}^{(p-1)/2}=\overline{x}^{2^rt}=\overline{\left(\frac{x}{p}\right)}=\overline{-1}=(\overline{x}^t)^{2r}$. Il suffit donc de trouver un non-résidu quadratique. D'après la proposition suivante, prendre des éléments de $(\Z/p\Z)^*$ au hasard et tester si ce sont des résidus quadratiques aboutit relativement vite à une solution. Par exemple au bout de $10$ essais, la probabilité de ne pas en avoir trouvé est $(1/2)^{10}<1/1000$. 




\begin{Prop}
Soit $p\in \mathbb{P}_{\geq 3}$. Alors il y a exactement $\frac{p-1}{2}$ carrés dans $(\Z/p\Z)^*$. 
\end{Prop}

\begin{proof}
Soit $f:(\Z/p\Z)^*\rightarrow (\Z/p\Z)^*$ défine par $f(x)=x^2$, pour $x\in (\Z/p\Z)^*$. Alors $f$ est un morphisme de groupes et $\ker(f)=\{\pm \overline{1}\}$, d'après le corollaire~\ref{corRacines_carrées_moins_un}. On a donc \begin{equation}\label{eqCardinal_quotient_image}
|\mathrm{im}(f)||\ker(f)|=|(\Z/p\Z)^*|=p-1=2|\mathrm{im}(f)|.
\end{equation}

En effet, soit $y\in \mathrm{im}(f)$ et $x_y\in (\Z/p\Z)^*$ tel que $f(x_y)=y$. Soit $x'\in (\Z/p\Z)^*$. Alors $f(x')=y$ si et seulement si $f(x')=f(x_y)$ si et seulement si $f(x'(x_y)^{-1})=f(\overline{1})=\overline{1}$ si et seulement si $x'\in x_y\ker(f)$. On a donc $(\Z/p\Z)^*=\bigsqcup_{y\in \mathrm{im}(f)} x_y\ker(f)=\ker(f)\bigsqcup_{y\in \mathrm{im}(f)} x_y$, d'où \eqref{eqCardinal_quotient_image}. 
\end{proof}
\end{comment}


 
\section{Recherche de racines dans $\Z/p^\alpha\Z$}

Soient $p\in\mathbb{P}_{\geq 3}$, $\alpha\in \N^*$ et $a\in \Z$. On cherche une racine de $a$ modulo $p^\alpha$. Nous allons partir d'une racine carrée de $a$ modulo $p$, puis nous allons en déduire une racine carrée modulo $p^2$, modulo $p^4$, etc. Par simplicité, nous nous intéresserons au cas où $a$ est inversible modulo $p$. 

Nous nous basons sur le lemme suivant, qui est une version simplifiée du lemme de Hensel, qui permet de trouver une racine d'un polynôme de $\Z[X]$ modulo $p^{2n}$ lorsque l'on connaît une racine de $P$ modulo $p^n$ qui n'est pas une racine de $P'$ modulo $p$.

\begin{lemme}\label{lemHensel}(Hensel)
Soient $P\in  \Z[X]$ et $n\in \N^*$. On suppose qu'il existe $c\in \Z$ tel que $P(c)\equiv 0[p^n]$ et $P'(c)\not \equiv 0[p]$. Alors il existe $c'\in \Z$, unique modulo $p^{2n}$, tel que $c'\equiv c [p^n]$ et  $P(c')\equiv 0[p^{2n}]$. 
\end{lemme}

\begin{proof}
D'après la formule de Taylor à l'ordre $1$ en $c$, il existe $R\in \Z[X]$ tel que \[P(X)=P(c)+(X-c)P'(X)+(X-c)^2 R(X).\] On cherche $c'$ sous la forme $c'=c+kp^n$, où $k\in \Z$.  Écrivons $P(c)=p^n\ell$, où $\ell\in \Z$.   En évaluant en $c'$, on obtient \begin{equation}\label{eqTaylor_c'}
P(c')=P(c)+kp^n P'(c')+p^{2n}k^2 R(c')=p^n\left(\ell+k P'(c')\right)+p^{2n}k^2 R(c').
\end{equation}

Comme $c'\equiv c[p^n]$, on a $P'(c)\equiv P'(c')[p^n]$, donc \begin{equation}\label{eqTaylor_c'}
P(c')\equiv P(c)+kp^n P'(c)[p^{2n}]\equiv p^n(\ell+k P'(c))[p^{2n}].
\end{equation}

On a donc $P(c')\equiv 0[p^{2n}]$ si et seulement si $p^n (\ell+kP'(c))\equiv 0[p^{2n}]$ si et seulement si $\ell+kP'(c)\equiv 0[p^n]$. 

Comme $P'(c)\not \equiv 0[p]$, $p$ ne divise pas $P'(c)$ donc $p^n\wedge P'(c)=1$, donc $P'(c)$ est inversible modulo $p^n$. Il existe donc $k\in \Z$ (unique modulo $p^n$) tel que $\ell+kP'(c)\equiv 0[p^n]$ donc tel que $P(c')\equiv 0[p^{2n}]$. 
\end{proof}

\begin{ex}
On cherche une racine $\gamma$ de $35$ modulo $43^2$. On a vu que $11$ est une racine de $35$ modulo $43$, on cherche donc $\gamma$ sous la forme $\gamma=11+43k$, où $k\in \Z$. On cherche donc $k\in \Z$ tel que $(11+43k)^2\equiv 35 [43]$. On a alors $121+2.11.43k+43^2.k^2\equiv 35[43^2]\equiv 121+2.11.43k$. On a donc $2.11.43k\equiv 121-35=-86=(-2).43[43^2]$. On a donc $2.11k\equiv -2[43]$, soit $11k\equiv -1[43]$ et on constate que cette condition est vérifiée si et seulement $k=-4$. On en déduit que $11-4.43=-161$ et donc $161$ sont des racines de $35$ modulo $43^2$.
\end{ex}

\begin{remarque}
Avec les notations du lemme, supposons que l'on cherche $c'\in \Z$ tel que $P(c')\equiv 0[p^{2n}]$. Alors $P(c')\equiv 0[p^n]$ donc $c'$ est une racine de $P$ modulo $p^n$. Il est donc naturel de chercher $c'$ congru à une racine de $P$ modulo $p^n$ dans le lemme. 
\end{remarque}

 
\begin{lemme}\label{lemExistence_racine_modulo_pAlpha_inversible}
Soit $a\in \Z$ et $p\in \mathbb{P}_{\geq 3}$. On suppose que $\left(\frac{a}{p}\right)=1$. Alors pour tout $\alpha\in \N^*$, il existe $\gamma\in \Z$ tel que $\gamma^2\equiv a[p^n]$.
\end{lemme}

\begin{proof}
  Soit $P=X^2-a\in \Z[X]$. Soit $\gamma_0\in \Z$ tel que $P(\gamma_0)\equiv 0[p]$. Alors $P'(\gamma_0)=2\gamma_0$. Comme $p$ ne divise pas $a=\gamma_0^2$, $p$ ne divise pas $\gamma_0$. Par récurrence, en utilisant le lemme~\ref{lemHensel}, on en déduit une suite $(\gamma_k)\in \Z^{\N^*}$ telle que pour tout $k\in \N^*$, on ait $\gamma_{k+1}\equiv \gamma_k [p^{2^k}]$ et $P(\gamma_k)\equiv 0[p^{2^k}]$. 
  
  Soit $k\in \N$ tel que $2^k\geq n$. Alors il existe $m\in \Z$ tel que $\gamma_k^2+m p^{2^k}=a=\gamma_k^2+p^n.p^{2^k-n}m$, donc $\gamma_k$ est une racine carrée de $a$ modulo $p^n$. 
\end{proof}



\begin{comment}
\begin{lemme}(admis, cf \cite[I.5.1.4]{demazure2008cours})
Soit $a\in \Z$ impair et $\alpha\in \N_{\geq 3}$. Alors $a$ est un résidu quadratique modulo $2^\alpha$ si et seulement si $a\equiv 1[8]$.
\end{lemme}
...
\begin{definition}
Soient $A$ un anneau et $\omega:K\rightarrow \Z\cup \{\infty\}$. On dit que $\omega$ est une valuation si on a, pour tous $x,x'\in A$:  \begin{enumerate}
...
\begin{definition}
Soit $p\in \mathbb{P}$. On définit la valuation $p$-adique $\omega_p:\Q\rightarrow \Z\cup \{\infty\}$ de la façon suivante. Si $x=0$, on pose $\omega_p(x)=-\infty$. Si $x\in \Q\setminus \{0\}$, on écrit $x=p^k x' $, avec $k\in \Z$ $x'=\frac{u}{v}$, avec $(u,v)\in \Z\times \Z\setminus \{0\}$ et $u,v$ premiers avec $p$. On pose alors $\omega_p(x)=k$.
\end{definition}

On peut vérifier (exercice) que si $p\in \mathbb{P}$, alors $\omega_p$ est une valuation sur  $\Z$. 


\begin{lemme}\label{l_val}(voir TD)

Soient $A$ un anneau et $\omega:A\rightarrow \Z\cup \{\infty\}$ une valuation. Soient $x,x'\in A$ tels que $\omega(x)\neq \omega(x')$. Alors $\omega(x+x')=\min(\omega(x),\omega(x'))$.

\end{lemme}


\begin{Prop}
Soient $p\in \mathbb{P}_{\geq 3}$ , $\alpha\in \N^*$ et $a\in \Z\setminus p^\alpha\Z$.  On écrit $a=p^k a'$, où $k=\omega_p(a)\in \llbracket 0,\alpha-1\rrbracket$ et $a'\in \Z$ est premier avec $p$. Alors $a$ est un carré modulo $p^\alpha$ si et seulement les conditions suivantes sont vérifiées : \begin{enumerate}
\item $\left(\frac{a'}{p}\right)=1$, autrement dit, $a$ est un carré modulo $p$,
...
\begin{proof}
On suppose que $a$ admet une racine $\gamma$ modulo $p^\alpha$. On a $p^\alpha | a-\gamma^2$, donc $\omega_p(p^\alpha)=p^\alpha\geq \omega(a-\gamma^2)$. 

Supposons que $\omega_p(a)<\omega_p(\gamma^2)$. Alors d'après le lemme~\ref{l_val}, $\omega_p(a-\gamma^2)=\omega(a)$, donc $\alpha\leq \omega_p(a)$ donc $p^\alpha$ divise $a$: c'est absurde par hypothèse.


 Supposons que $\omega_p(a)>\omega_p(\gamma^2)$. Alors d'après le lemme~\ref{l_val}, on a $\omega_p(a-\gamma^2)=\omega_p(\gamma^2)$, donc $\alpha\leq \omega_p(\gamma^2)< \omega_p(a)$. C'est aussi absurde. On en déduit que $\omega_p(a)=\omega_p(\gamma^2)=2\omega_p(\gamma)$. On en déduit que $k$ est pair.
 
 Soient $\ell=\omega(\gamma)$ et $\gamma'=p^{-\ell}\gamma\in \Z$. Alors $\gamma^2=p^{k}\gamma'^2\equiv p^k a'[p^\alpha]$, donc $\gamma'^2\equiv a'[p^{\alpha-k}]$ donc $a'$ est un carré modulo $p^{\alpha-k}$. En particulier, $\gamma$ est un carré modulo $p$. On a bien 1 et 2. 
 
 Réciproquement, supposons que 1 et 2 sont satisfaites. D'après le lemme~\ref{lemExistence_racine_modulo_pAlpha_inversible}, il existe une racine $\gamma$ de $a'$ modulo $p^{\alpha-k}$. Soit $\gamma_0$ une racine de $a'$ modulo $p^{\alpha-k}$. Alors $p^{\alpha-k}|(\gamma^2-a')$ donc $p^\alpha| ((\gamma p^{k/2})^2-a'p^k)$ donc $\gamma p^{k/2}$ est une racine de $a$ modulo $p^\alpha$. 
 \end{proof}
...
\section{Recherche de racines carrées dans $\Z/n\Z$}


Soit $n\in \N_{\geq 2}$ et $a\in \Z$. On suppose que l'on sait décomposer $n$ en produit de facteurs premiers. On écrit $n=p_1^{\alpha_1}\ldots p_k^{\alpha_k}$, où $k\in \N$, $\alpha_1,\ldots,\alpha_k\in \N^*$ et où les $p_i$ sont distincts deux à deux.  Alors $a\mathrm{\ mod\ }n$ est un carré si et seulement si $a\mathrm{\ mod\ }p_i^{\alpha_i}$ est un carré pour tout $i\in \llbracket 1,k\rrbracket$. En effet, supposons que $a\mathrm{\ mod\ }n=\gamma^2\mathrm{\ mod\ }n$. Alors \[\phi(a\mathrm{\ mod\ }n)=(a\mathrm{\ mod\ }p_1^{\alpha_1},\ldots,a\mathrm{\ mod\ }p_k^{\alpha_k})=\phi(\gamma\mathrm{\ mod\ }n)^2=(\gamma^2\mathrm{\ mod\ }p_1^{\alpha_1},\ldots,\gamma^2\mathrm{\ mod\ }p_k^{\alpha_k}),\] donc $a\mathrm{\ mod\ }p_i^{\alpha_i}$ est un carré pour tout $i\in \llbracket 1,k\rrbracket$. Réciproquement, si $a\mathrm{\ mod\ }p_i^{\alpha_i}=\gamma_i^2\mathrm{\ mod\ }p_i^{\alpha_i}$ pour tout $i\in \llbracket 1,k\rrbracket$, alors pour tout $\gamma\in  \Z$ tel que $\gamma\mathrm{\ mod\ }n=(\gamma_1\mathrm{\ mod\ }p_1^{\alpha_1},\ldots,\gamma_k\mathrm{\ mod\ }p_k^{\alpha_k})$, alors $\gamma$ est une racine de $a$ modulo $n$. Pour trouver les racine de $a$ modulo $n$, il suffit donc de trouver les racines de $a$ modulo $p_i^{\alpha_i}$ pour tout $i\in \llbracket 1,k\rrbracket$, puis de déterminer leur image réciproque par l'isomorphisme chinois. 

\begin{ex}
On se place dans $\Z/154\Z$. On se demande si $\overline{23}$ est un carré. On a $154=2.7.11$, il faut donc vérifier si $23$ est un carré modulo $7$ et modulo $11$ (car tout élément de $\Z/2\Z$ est un carré). On a $\left(\frac{23}{7}\right)=\left(\frac{2}{7}\right)=1$ car $7\equiv -1[8]$.  On a $\left(\frac{23}{11}\right)=\left(\frac{1}{11}\right)=1$, donc $23$ est un carré modulo $7$ et $11$, c'est donc un carré modulo $154$. On constate que $(3\mathrm{\ mod\ }7)^2=(2\mathrm{\ mod\ }7)=23\mathrm{\ mod\ }7$. Soit $\phi:\Z/154\Z\rightarrow \Z/2\Z\times \Z/7\Z\times \Z/11\Z$ l'isomorphisme chinois. Alors $\phi^{-1}(1\mathrm{\ mod\ }2,2\mathrm{\ mod\ }7,1\mathrm{\ mod\ }11)$ est une racine de $\overline{23}$. Grâce à l'algorithme de Garner, on obtient que $\overline{45}$ est une racine de $\overline{23}$.
\end{ex}

\begin{remarque}
On a montré comment déterminer une racine modulo $n$ si l'on sait factoriser $n$ et si tous ses facteurs premiers sont congrus à $3$ modulo $4$. Des méthodes similaires permettent de trouver des racines carrées dans les cas restants (voir \cite{demazure2008cours}). Comme on ne connaît pas d'algorithme «~rapide~» de factorisation des entiers, on peut se demander s'il n'est pas possible de se dispenser de cette factorisation. En fait,  ce n'est pas possible,  car si on connaît une méthode pour extraire rapidement les racines carrées d'un nombre modulo $n$, alors on peut trouver un diviseur non trivial de  $n$. En effet, supposons que $n$ n'est pas primaire (c'est à dire que ce n'est pas la puissance d'un nombre premier, autrement dit $k\geq 2$). Soit $a\in \Z$ tel que $a\wedge n =1$. Alors si $\epsilon_1,\ldots,\epsilon_k\in \{-1,1\}$ et  $\gamma=\phi^{-1}((-1)^{\epsilon_1} a\mathrm{\ mod\ }p_1^{\alpha_1},\ldots,(-1)^{\epsilon_k} a\mathrm{\ mod\ }p_k^{\alpha_k})$, $\gamma$ est une racine carrée de $a^2\mathrm{\ mod\ }n$. On peut donc trouver une racine carrée $\gamma$ de $a^2$ modulo $n$, qui est différente de $\pm a$, modulo $n$. Alors $a^2\equiv \gamma [n]$, donc $(a-\gamma)(a+\gamma)\equiv 0[n]$ donc $n$ divise $(a-\gamma)(a+\gamma)$. Soit $d=(a-\gamma)\wedge n$. Si $d=1$, alors par le lemme de Gauss, $n$ divise $a+\gamma$, ce qui est absurde par choix de $\gamma$. Ainsi, $1<d<n$ et on a trouvé un diviseur non trivial de $n$. On a donc montré que connaître une méthode pour extraire les racines carrées de tous les nombres modulo $n$ est aussi difficile que de savoir factoriser $n$. 
