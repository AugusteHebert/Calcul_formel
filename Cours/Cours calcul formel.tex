...
\begin{Prop}
Soient $p\in \mathbb{P}_{\geq 3}$ , $\alpha\in \N^*$ et $a\in \Z\setminus p^\alpha\Z$.  On écrit $a=p^k a'$, où $k\in \llbracket 0,\alpha-1\rrbracket$ et $a'\in \Z$ est premier avec $p$. Alors $a$ est un carré modulo $p^\alpha$ si et seulement les conditions suivantes sont vérifiées : \begin{enumerate}
...
\begin{proof}
Si $k=0$, le résultat est vrai, d'après le lemme~\ref{lemExistence_racine_modulo_pAlpha_inversible}. On suppose donc $k\geq 1$. 

On démontre le résultat par récurrence sur $\alpha$. Si $\alpha=1$, on a nécessairement $k=0$ et le résultat découle de la définition du symbole de Legendre.  Soit $\alpha\in \N_{\geq 2}$. On suppose le résultat vrai pour tout $\alpha'\in \llbracket 1,\alpha-1\rrbracket$. 

Supposons que $a$ est un carré modulo $p^\alpha$. Soit $\gamma$ une racine de $a$ modulo $p^\alpha$. Alors \begin{equation}\label{e_div}
p\alpha\mid  p^k a'-\gamma^2.
\end{equation} Comme $\alpha>1$, on en déduit que $p|p^ka'-\gamma^2$ et comme $k>0$, $p|\gamma^2$. Comme $p$ est premier, cela implique que $p$ divise $\gamma$. On peut donc écrire $\gamma=p \gamma'$. D'après \eqref{e_div}, comme $\alpha\geq 2$, on a $p^2| (p^{k}a'-p^2\gamma')$, donc $p^2|p^{k}a'$. Comme $a'\wedge p=1$, $a'\wedge p^2=1$, donc d'après le lemme de Gauss, $p^2|p^k$, donc $k\geq 2$. L'équation \eqref{e_div} devient $p^{\alpha-2}|(p^{k-2}a'-\gamma'^2)$, donc $p^{k-2}a'$ est un carré modulo $p^{\alpha-2}$. Comme $\alpha>k$, $\alpha-2\geq 1$. On peut donc appliquer l'hypothèse de récurrence et on en déduit que 1 et 2 sont satisfaites.
