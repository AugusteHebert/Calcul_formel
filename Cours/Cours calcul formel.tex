...
Pour cela, on se base sur le lemme suivant. Si $x\in \Z$ et $\ell\in \N$, on note $x[\ell]$ l'image de $x$ dans $\Z/\ell\Z$. On note $f:\Z/n\Z\rightarrow \Z/p\Z\times \Z/q\Z$ l'isomorphisme chinois, définit par $f(x[n])=(x[p],x[q])$ pour $x[n]\in \Z/n\Z$. 
...
Soit $X=\{a\in G^\times\mid a^t\neq \overline{1}\}$. Alors $|X|\geq \frac{3}{4}\varphi$.
\end{lemme}

\begin{proof}
Soit $H=\{a\in G\mid a^t=\overline{1}\}$. Alors $H$ est un sous-groupe de $G$. Soit $f:G\rightarrow (\Z/p\Z)^\times \times (\Z/q\Z)^{\times}$ définie par $f(x[n])=(x[p],x[q])$, pour $x\in \Z$ tel que $x\wedge n =1$. Soient $\epsilon_1=f^{-1}((-1[p],1[q])$ et $\epsilon_2=f^{-1}((1[p],-1[q]))=-\epsilon_1$. Comme $t$ est impair, on a $(1[n])^t=1[n]$, $(-1[n])^t=-1[n]$, $\epsilon_1^t=f^{-1}((-1[p],1[q])^t)=f^{-1}((-1[p],1[q]))=\epsilon_1$ et $\epsilon_2^t=f^{-1}(1[p],-1[q])^t)=f^{-1}(1[p],-1[q])=\epsilon_2$. On en déduit que les images de $1[n],-1[n],\epsilon_1$ et $\epsilon_2$ dans $G/H$ sont distinctes. En particulier, $|G/H|\geq 4$, donc $|H|\leq 1/4 |G|$. On a $X=G\setminus H=\bigsqcup_{gH\in  G/H\setminus \{H\}}gH$, donc \[|X|=(|G/H|-1)|H|=|G|-|H|\geq 3/4 |G|.\]
\end{proof}
...
Soit \[Y= \{a\in X\mid \exists j\in \llbracket 1,s\rrbracket, a^{2^{j-1}t}\neq \pm \overline{1}, a^{2^jt}=\overline{1}.\] Si $j\in \llbracket 1,s\rrbracket$, on pose $Y_j=Y\cap Y_j$. 
...
\begin{proof}
Soit  $a\in X_j$. Soit $b=a^t\in G$.  On a $b^{2^{j-1}}\neq \overline{1}$, donc $f(b^{2^{j-1}})\neq (1[p],1[q])$. Autrement dit,  $b^{2^{j-1}}[p]\neq 1[p]$ ou $b^{2^{j-1}}[q]\neq 1[q]$. On a donc $b[p]$ d'ordre $2^j$ ou $b[q]$ d'ordre $2^j$. On a donc $2^j| (p-1)$ ou $2^j|(q-1)$, d'après le théorème de Lagrange. 
\end{proof}
...
\begin{proof}
Soit $j\in \llbracket 1,s\rrbracket$ tel que $X_j\neq \emptyset$. Comme $((\Z/q\Z)^\times,.)$ est cyclique d'ordre $q-1$, on peut choisir un générateur $\alpha$ de $(\Z/q\Z)^\times$. Soit $\beta=\alpha^{2^{s_q-j}t_q}\in (\Z/q\Z)^\times$ (par le lemme~\ref{l_ineq_j}, $s_q-j\geq 0$). Alors $\beta$ est d'ordre $2^j$. On a donc $(\beta^{2^{j-1}})^2=1[q]$ et $\beta^{2^{j-1}}\neq 1[q]$. Comme $\Z/q\Z$ est un corps, on en déduit que $\beta^{2^{j-1}}=-1[q]$. 

 Soit $\eta_j=f^{-1}(1[p],\beta)\in G$. Montrons que $|Y_j|\geq |X_j|/2$. Si $X_j=Y_j$, c'est clair. Supposons que $X_j\neq Y_j$. Soit $a\in X_j\setminus Y_j$. On a $a^{2^{j-1}t}=-1[n]$. On a $(a\eta_j)^{2^jt}=a^{2^jt}=1[n]$. On a également $(a\eta_j)^{2^{j-1}t}=a^{2^{j-1}t} (\eta_j)^{2^{j-1}t}$. On a  \[f(\eta_j^{2^{j-1}t})=(1[p],\beta^{2^{j-1}t})=(1[p],(-1)^t[q])=(1[p],-1[q]),\] donc $\eta_j^{2^{j-1}t}=f^{-1}(1[p],-1[q])$. On a donc $f((a\eta_j)^{2^{j-1}t})=(-1[p],1[q])\neq f(-1[n])$, donc $a\eta_j\in Y_j$. On a donc montré : \[\eta_j \cdot (X_j\setminus X_j)\subset Y_j,\] donc $|Y_j|\geq 1/2 |X_j|$.  
...
Soit $a\in (\Z/n\Z)^\times$. Alors la probabilité que $a^t\neq 1$ et qu'il existe $j\in \llbracket 1,s\rrbracket$ tel que $a^{2^{j-1}t}\neq \pm\overline{1}$ et $a^{2^jt}=\overline{1}$ est supérieure ou égale à  $\frac{3}{8}$.
\end{lemme}
...
Soit $p\in \mathbb{P}$ et $\alpha\in \N^*$. Nous allons commencer par décrire un algorithme qui permet de trouver une racine carrée de $a$ modulo $p$. Nous verrons ensuite comment en déduire une racine carrée de $a$ modulo $p^\alpha$.
...
Dans le cas $p\neq 2$, nous nous basons sur le lemme suivant, qui est une version simplifiée du lemme de Hensel, qui permet de trouver les racines d'un polynôme de $\Z[X]$ modulo $p^{2n}$ lorsque l'on connaît une racine de $P$ modulo $p^n$ qui n'est pas une racine de $P'$ modulo $p$.
...
\begin{proof}
  Soit $P=X^2-a\in \Z[X]$. Soit $\gamma_0\in \Z$ tel que $P(\gamma_0)\equiv 0[p]$. Alors $P'(\gamma_0)=2\gamma_0$. Comme $p$ ne divise pas $a$, $p$ ne divise pas $\gamma_0$. Par récurrence, en utilisant le lemme~\ref{lemHensel}, on en déduit une suite $(\gamma_k)\in \Z^{\N^*}$ telle que pour tout $k\in \N^*$, on ait $\gamma_{k+1}\equiv \gamma_k [p^{2^k}]$ et $P(\gamma_k)\equiv 0[p^{2^k}]$. 
