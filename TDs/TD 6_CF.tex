...
\item[$2.$] Déterminer l'ordre de $2$ dans les groupes (multiplicatifs) $(\mathbb{Z}/11 \mathbb{Z})^{\times}$, $(\mathbb{Z}/31 \mathbb{Z})^{\times}$ et $(\Z/341\Z)^\times$ (on pourra faire un calcul direct ou bien remarquer que $341=11\times 31$ et utiliser le théorème chinois). 
...
\item[$7.$] Montrer qu'un nombre de Fermat $F_n=2^{2^n}+1  $ composé est un nombre de Poulet (on pourra commencer par montrer que $(2^{2^n}+1)|(2^{2^n+1}-1)|\ldots |(2^{2^{n+\ell}}-1)$, pour tout $\ell\in \N^*$).
\end{itemize}
\end{ex}
...
\end{ex}
 ...
 \begin{ex}
 Soit $p$ un nombre premier supérieur ou égal à $3$. On veut montrer: \begin{equation}\label{e_rel}
  \forall a\in \Z, \left(\frac{a}{p}\right)\equiv a^{(p-1)/2}[p],
 \end{equation}
  \begin{enumerate}
 \item Montrer \eqref{e_rel} si $a\in p\Z$.
  
 
 
 
 \item Soit $G=(\Z/p\Z)^\times$. Soit $f:G\rightarrow G$ définie par $f(x)=x^2$, pour tout $x\in G$. Montrer que $f$ est un morphisme de groupes.
 
 \item Déterminer $\ker(f)$. Soit $y\in \mathrm{Im}(f)$. Quel est le nombre d'antécédents de $y$ par $f$ ? Déterminer $|\mathrm{Im}(f)|$.
 
 \item On pose $P=X^{(p-1)/2}-1\in \mathbb{F}_p[X]$.  Montrer que $P(y)=0$, pour tout $y\in \mathrm{Im}(f)$. 
 
 \item Quelles sont les valeurs possibles de $P(y)$, si $y\in \mathbb{F}_p$. 
 
\item On admet que si $K$ est un corps, tout polynôme non-nul $Q\in K[X]$ a au plus $d$ racines, où $d$ est le degré de $Q$. En déduire \eqref{e_rel}.
 
\item Soit $A$ un anneau intègre et soit $Q\in A[X]\setminus \{0\}$. Le polynôme $Q$ peut-il avoir au moins $d+1$ racines, où $d=\mathrm{deq}(Q)$ ?

\item Soit $A$ un anneau. Donner une condition nécessaire et suffisante sur $A$ pour que la condition suivante soit satisfaite : pour tout $Q\in A[X]\setminus \{0\}$, $Q$ a au plus $\mathrm{deg}(Q)$ racines.
 \end{enumerate}
 \end{ex}
