...
\begin{ex}\label{exponentiation_rapide}
En cours, nous avons vu comment calculer $(a\mathrm{\ mod\ }b)^n$ grâce à une méthode rapide qui nécessitait le calcul de la représentation binaire de $b$. Le but de cet exercice sera de reformuler cette méthode grâce à une représentation ternaire. On suppose qu'on dispose de la fonction $a\%b$ qui détermine le reste de la division euclidienne de $a$ par $b$. 

\begin{itemize}
\item[$1.$] Écrire 87 en base 3.
\item[$2.$] Écrire une fonction qui prend en entrée un entier $n$ et renvoie sa décomposition en base $3$.
%\textit{Indication: On se servira d'une propriété sur le symbole de Legendre}
\item[$3.$] Rappeler l'algorithme d'exponentiation modulaire qui, à partir de trois entiers $(a,b,n)\in \mathbb{N}^* \times \mathbb{N}^2$, calcule $(a\mathrm{\ mod\ }b)^n$ grâce à la représentation binaire  de $b$.
\item[$4.$] En déduire une fonction itérative qui, prenant en entrée trois entiers $(a,b,n)\in \mathbb{N}^* \times \mathbb{N}^2$, calcule $(a\mathrm{\ mod\ }b)^n$ en admettant qu'on connaisse, grâce à la question $2$, la décomposition en base 3 de $b$.
\item[$5.$] Réécrire l'algorithme précédent de façon récursive (sans nécessairement utiliser cette fois-ci la décomposition en base 3 de $b$). 

\item[$6.$] Soit $N\in \Z$ et $a\in \Z/N\Z$. On chercher à calculer $a^{87}$. Combien de produits doit on calculer dans $\Z/N\Z$ en utilisant l'algorithme d'exponentiation rapide classique. Combien avec l'algorithme de la question 4. Même question avec $a^{81}$ et $a^{128}$. 
...
\begin{ex}\label{caractérisation_carrés}
Soit $p\in \mathbb{P}\setminus\{2\}$. On veut montrer que l'ensemble des carrés non nuls de $\Z/p\Z$ est l'ensemble $\{x\in \Z/p\Z|x^{(p-1)/2}=1\}$. \begin{enumerate}

\item Soit $x\in (\Z/p\Z)^*$. Montrer que si $x$ est un carré, alors $x^{(p-1)/2}=1$.

\item Soit $f:(\Z/p\Z)^*\rightarrow (\Z/p\Z)^*$ définie par $f(x)=x^2$, pour $x\in (\Z/p\Z)^*$. Montrer que si $\alpha\in (\Z/p\Z)^*$, alors $|f^{-1}(\{\alpha\})|\in \{0,2\}$. 

\item En déduire que $|f\big((\Z/p\Z)^*\big)|=\frac{p-1}{2}$.

\item En déduire que l'ensemble des carrés non nuls de $\Z/p\Z$ est l'ensemble $\{x\in \Z/p\Z|x^{(p-1)/2}=1\}$. (on pourra utiliser l'exercice précédent)

\item Donner une condition nécessaire et suffisante sur $p$ pour que $-1$ soit un carré.
...
Exercice~\ref{exponentiation_rapide}

1. On a $87=81+3.2$ donc $87=10020_3$


2. 

\begin{tabular}{ll}
\textbf{Algorithme} & ternaire($a$)\\
		           & $L=[\ ]$\\
		           & $b=a$\\
		           & tant que $b>0$:\\
		                & \ \ \ {\rm | }$c=b\%3$\\
...
4.
 ...
 \begin{tabular}{ll}
\textbf{Algorithme} &expo\_rapide\_ternaire($a,n$)\\
		           & $L=\mathrm{ternaire}(n)$\\
		           & $P=a$\\
		           & $k=\mathrm{len}(L)$\\
		           & $b=1$\\
...
5.
 ...
 \begin{tabular}{ll}
\textbf{Algorithme} &expo\_rapide\_recursif($a,n$)\\
		           & si $n\%3==0$:\\
		                & \ \ \ {\rm | }Renvoyer  expo\_rapide\_recursif($a,n//3)^3$ \\		
		           & si $n\%3==1$:\\
		                & \ \ \ {\rm | }Renvoyer  expo\_rapide\_recursif($a,(n-1)//3)^3.a$ \\		
		           & si $n\%3==2$:\\
		                & \ \ \ {\rm | }Renvoyer  expo\_rapide\_recursif($a,(n-2)//3)^3.a^2$ \\		
\end{tabular}


6. On a $a^{87}=(((a^3)^3)^3)^3.(a^3)^2$. Calculer un cube coûte deux multiplications (car $x^3=(x.x).x$). Il faut calculer $((((a^3)^3)^3)^3$, ce qui coûte $2.4$ multiplications. Puis multiplier par $(a^3)^2$, ce qui fait deux autres multiplications (car on a déjà calculé $a^3$). On utilise donc $10$ multiplications.

On a $87=2^6+2^4+2^2+2+1=1010111_2$, donc $a^{87}=(((((a^2)^2)^2)^2)^2)^2).((((a^2)^2)^2)^2).((a^2)^2).a^2.a$. Il faut d'abord calculer $(((((a^2)^2)^2)^2)^2)^2)$, ce qui fait $6$ multiplications (une élévation au carré compte pour une multiplication). Au passage on aura calculé $((((a^2)^2)^2)^2)$, $((a^2)^2)$, $a^2$ et $a$. Il faut ensuite faire $3$ multiplications.

On a $81=3^4$, il faut donc faire $8$ multiplications pour calculer $a^{81}=(((a^3)^3)^3)^3$. On a $81=64+16+1=1010001_2$, il faut donc $8$ multiplications pour calculer $a^{81}$ par l'algorithme d'exponentiation rapide usuel.

On a $128=81+27+18+2=11202_3$. Il faut donc $4.2+1+2+2=13$ multiplications par l'algorithme d'exponentiation rapide ternaire.
On a $128=2^7$, il faut donc $7$ multiplications pour calculer $a^{128}$ par l'algorithme d'exponentiation rapide usuel.




Exercice~\ref{nombre_racines_polynôme}

1. On a $P(\alpha)=B(\alpha).0+R(\alpha)$. De plus $R$ est constant donc $R(\alpha)=R$. On a donc $R=P(\alpha)$. Si $P(\alpha)=0$, on a donc $P=(X-\alpha)B$, donc $(X-\alpha)$ divise $B$.

2. Soit $P\in K[X]$. Si $P$ est de degré $0$, $P$ est constant, donc $P$ n'a pas de racine. Soit $n\in  \N$. Supposons que tout polynôme de degré $n$ a au plus $n$ racines. Soit $P$ un polynôme de degré $n+1$. Supposons que $P$ admette une racine $\alpha$. Alors on peut écrire $P=(X-\alpha)B$, où $B\in K[X]$ et $B$ est de degré $n$. Soit $x\in K$. Alors $P(x)=0$ si et seulement si $x-\alpha=0$ ou $B(\alpha)=0$. On a donc $P^{-1}(\{0\})\subset \{\alpha\}\cup B^{-1}(\{0\})$, donc $P$ a au plus $n+1$ racines.

3. Les racines de $P$ sont $0$ et $2$ donc $P$ a deux racines. En fait, si $K=A$ est un anneau commutatif, on peut montrer que le résultat précédent reste vrai si et seulement si $A$ est intègre.


Exercice~\ref{caractérisation_carrés}

1. Si $x$ est un carré non nul, on peut écrire $x=\alpha^2$, pour un certain $\alpha\in \Z/p\Z^*$. Par le petit théorème de Fermat, $x^{(p-1)/2}=\alpha^{p-1}=\overline{1}$.

2. Soit $\alpha\in (\Z/p\Z)^*$. Supposons que $\alpha$ soit un carré. Alors il existe $\beta\in \Z/p\Z^*$ tel que $\beta^2=\alpha$. Soit $x\in \Z/p\Z$. Alors $x^2=\alpha$ si et seulement si $x^2-\beta^2=0$ si et seulement si $(x-\beta)(x+\beta)=0$. On a donc $f^{-1}(\{\alpha\})=\{\beta,-\beta\}$. De plus, $2$ est premier avec $p$, donc $\overline{2}$ est inversible dans $\Z/p\Z$, donc $2\beta=\beta+\beta\neq 0$. On en déduit que $\beta\neq -\beta$ donc $|f^{-1}(\{\alpha\})|=2$.

3. Soit $X=f(\Z/p\Z^*)$. Si $x\in \Z/p\Z^*$, alors $x\in f^{-1}(\{f(x)\})$. On a donc $\Z/p\Z^*=\bigsqcup_{\alpha\in X} f^{-1}(\{\alpha\})$. On a donc $|\Z/p\Z^*|=p-1=\sum_{\alpha\in X} |f^{-1}(\{\alpha\})|=\sum_{\alpha\in X} 2=2|X|$, donc $X=\frac{p-1}{2}$.

4. Soit $P=X^{(p-1)/2}-1$. Alors par l'exercice précédent, $P$ a au plus $\frac{p-1}{2}$ racines (car $\Z/p\Z$ est un corps). Par la question 1, tout élément de $X$ est une racine de $P$ et par la question 3, $X$ a $\frac{p-1}{2}$. L'ensemble $X$ est donc l'ensemble des racines de $P$, ce qu'on voulait démontrer.

5. On en déduit que $-1$ est un carré si et seulement si $\frac{p-1}{2}$ est pair si et seulement si $p\equiv 1[4]$. 




Exercice~\ref{indicatrice_d_Euler}
