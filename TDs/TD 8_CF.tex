...
\begin{ex}\label{e_val}
\begin{enumerate}
\item Soit $p\in \mathbb{P}$. Montrer que $\omega_p$ est une valuation de $\Z$. 

\item\label{q_som} Soit $\omega:K\rightarrow \Z\cup \{\infty\}$, où $K$ est un corps. Soient $x,x'\in K$. On suppose que $\omega(x)\neq \omega(x')$. Que vaut $\omega(x+x')$ ?
...
\item À quelle condition $\Z/p^\alpha\Z$ est-il intègre ?


\item Soit $a\in \Z$ tel que $a\wedge p=1$. On suppose que $a$ admet une racine modulo $p^\alpha$.  On veut montrer que $\overline{a}$ admet exactement deux racines dans $\Z/p^\alpha\Z$. \begin{enumerate}
\item Soit $\gamma\in \Z$ tel que $\overline{\gamma}$ soit une racine de $\overline{a}$ dans $\Z/p\Z$. Soit $X$ une indéterminée. Factoriser $X-\overline{\gamma}$ dans $\Z/p^\alpha\Z [X]$. 

\item Soit $x\in \Z$. Montrer que $(x-\gamma)\wedge (x+\gamma)$ divise $2\gamma$. 
...
\item Soit $a\in \Z\setminus p^\alpha \Z$. On écrit $a=p^k a'$. On s'intéresse aux racines de $\overline{a}$ dans $\Z/p^\alpha\Z$.  On suppose que $k$ est pair et que $\left(\frac{a'}{p}\right)=1$. \begin{enumerate}
\item Exprimer $k$ à l'aide de $\omega_p$. 

\item Soit $\gamma\in \Z$ telle que $\overline{\gamma}$ soit une racine de $\overline{a}$ dans $\Z/p^\alpha\Z$. Montrer que $\omega_p(\gamma)=1/2k$ (on pourra raisonner par l'absurde et utiliser la question~\ref{q_som} de l'exercice~\ref{e_val}).

\item Déterminer l'ensemble des racines de $a$ modulo $p^\alpha$, en fonction de celles de $a'$ modulo $p^{\alpha-k}$ (on pourra écrire $\gamma=p^{\omega_p(\gamma)}\gamma'$, où $\gamma'\in \Z$).

\item Combien $\overline{a}$ admet-il de racines dans $\Z/p^\alpha \Z$ ?
\end{enumerate}  
\item \begin{enumerate}
\item Déterminer l'ensemble des racines de $0$ modulo $p^\alpha$ (on pourra les caractériser à l'aide de $\omega_p$).

\item Montrer qu'il y en a $p^{\lfloor \alpha/2\rfloor}$.
\end{enumerate} 
\end{enumerate}
\end{ex}
