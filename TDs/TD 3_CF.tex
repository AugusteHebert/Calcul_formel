...
\begin{ex}
Déterminer les sous-groupes suivants: $\langle \overline{12},\overline{21}\rangle\subset \Z/100\Z$ et $\langle \overline{12},\overline{21}\rangle\subset \Z/30\Z$. 
\end{ex}
...
\begin{enumerate} \item Soit $p\in \mathbb{P}$. Déterminer $\{x\in \Z/p\Z\mid x^2=\overline{1}\}$.
...
\begin{ex}\label{systeme_non_premier}\
 Résoudre les systèmes $ \left \{ \begin{array}{l}
{x}\equiv 1 [2]\\
{x} \equiv  1[3]\\
x\equiv 3[4]\\
x\equiv 2[5]\\
...
x\equiv 2[5]\\
x\equiv 1[6]
\end{array} \right.   $, d'inconnue $x\in \Z$. 
...
\item Soient $p_1,\ldots,p_k\in \mathbb{P}$ et $n=p_1\ldots p_k$. On suppose les $p_i$ distincts. Montrer que si $u\in \Z/n\Z$ et $N=|\{x\in \Z/n\Z\mid  x^2=u\}|$, alors $N$ est soit nul, soit une puissance de $2$ inférieure ou égale à $2^k$ (on pourra utiliser le théorème  chinois). 
...
\begin{ex}
Soit $A$ un anneau intègre. Soit $x\in A$. On dit que $x$ est \textbf{irréductible} s'il est ni nul, ni inversible, ni produit de deux éléments  non inversibles. On dit que $x$ est \textbf{premier} s'il est non-nul et non-inversible et si pour tout $a,b\in A$ tels que $x$ divise $ab$, alors $x$ divise $a$ ou $x$ divise $b$.

\begin{enumerate}
\item Montrer que tout élément premier est irréductible.

\item Montrer que dans $\Z$, un élément est irréductible si et seulement si il est premier si et seulement si il est premier au sens donné dans le cours.

\item Soit $A=\Z[i\sqrt{3}]$. On définit $N:A\rightarrow \N$ par $N(z)=|z|^2$ pour tout $z\in A$ (où $|\cdot|$ désigne le module de $\mathbb{C}$). Donner l'expression de $N(a+ib\sqrt{3})$, pour $a,b\in \Z$.  Montrer que $N$ est bien à valeurs dans $\N$ et que $N(z z')=N(z)N(z')$ pour tous $z,z'\in A$.

\item Déterminer l'ensemble des éléments inversibles de $A$ (on pourra utiliser $N$).

\item Montrer que $4=2\times 2=(1+i\sqrt{3})(1-i\sqrt{3})$ et que $2, 1+i\sqrt{3}$, $1-i\sqrt{3}$ sont irréductibles.

\item Un anneau intègre $B$ est dit factoriel si pour tout élément $x\in B$, non-nul et non-inversible, on a :\begin{enumerate}
\item  il existe $n\in \N^*$ et $p_1,\ldots,p_n\in B$ des éléments irréductibles tels que $x=p_1\ldots p_n$,

\item si $m,n\in \N^*$, $p_1,\ldots,p_n,q_1,\ldots,q_m\in B$ sont irréductibles et $x=p_1\ldots p_n=q_1\ldots q_m$, alors $m=n$ et il existe une permutation $\sigma$ de $\llbracket 1,n\rrbracket$, $u_1,\ldots,u_n\in B^\times$ tels que $p_i=u_iq_{\sigma(i)}$ pour tout $i\in \llbracket 1,n\rrbracket$.
\end{enumerate}

\item L'anneau $A$ est-il factoriel ?
...
(1) Soit $d=(2n+1)\wedge n^2+n$. Alors $d$ divise $2(n^2+n)-n(2n+1)=n$, donc $d \mid (2n+1)\wedge n=1$, donc $d=1$.
...
On a $45\wedge 50=5\mid10$, donc l'équation admet des solutions. Soit $x\in \Z$. Alors $45x\equiv 0[50]$ si et seulement si $50$ divise $45 x$, ssi $10$ divise $9x$ ssi $10$ divise $x$. On a donc $\{\overline{x}\in \Z/50\Z\mid45 \overline{x}=\overline{0}\}=10 \Z/50\Z=\{\overline{0},\overline{10}, \overline{20}, \overline{30}, \overline{40}\}$ ou $\{x\in \mid45x\equiv 0[50]\}=10\Z$. 

Par Euclide étendu (ou directement), on obtient que $50\times 2-45\times 2 =10$, donc $\overline{x_0}=\overline{-2}$ est solution de l'équation. On obtient donc que $\{x\in \Z\mid45x\equiv 10[50]\}=-2+10\Z$. 
...
(2) Dans $\{x\in \Z/4\Z\mid\ x^2=\overline{1}\}=\{\overline{1},\overline{3}\}$. Soit $\phi:\Z/92\Z\rightarrow \Z/4\Z\times \Z/23\Z$ l'isomorphisme chinois. 

On a $4\times 6-23=1$. On a $\phi(\overline{24})=(\overline{0},\overline{1})$ et $\phi(\overline{-23})=(\overline{1},\overline{0})$. On a donc $\{x\in \Z/92\Z\mid x^2=1\}=\{\overline{1},\overline{-1},\overline{47},\overline{-47}\}$. 
