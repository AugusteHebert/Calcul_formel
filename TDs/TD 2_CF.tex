...
\begin{ex}
\begin{enumerate}
\item Est-ce que $2\Z\cup 3 \Z$ est un idéal de $\Z$ (à justifier).




\item Soient $a,b\in \Z$. Montrer que $a\Z+b\Z$ et $a\Z\cap b\Z$ sont des idéaux de $\Z$.

\item Déterminer $3\Z+7\Z$, $4\Z\cap 6\Z$, $4\Z+6\Z$. 
\end{enumerate}
...
Déterminer l'ensemble des  $u,v\in \Z$ tels que $899 u+493v=29$.
\end{ex}
...
\begin{ex}\label{exLemmeGauss}(lemme de Gauss)\
 Soient $a,b,c \in \mathbb{Z}\setminus\{0\}$. Montrer que 
\begin{itemize}
\item[$1)$] Si $a\mid bc$ et $a \wedge b=1$, alors $a \mid c$. Cela reste-t-il vrai en supprimant l'hypothèse $a\wedge b=1$ ?
\item[$2)$] Si $a \wedge b=1$, alors :\[a \mid c \text{ et }b \mid c  \Rightarrow ab \mid c.\]  Cela reste-t-il vrai en supprimant l'hypothèse $a\wedge b =1$ ?
...
\begin{ex}
\begin{enumerate}
\item Montrer que $I:=2\Z[X]+X\Z[X]$ est un idéal de $\Z[X]$.

\item Montrer que ce n'est pas un idéal principal.


\item Soit $A$ un anneau intègre. Montrer que si $A$ n'est pas un corps, alors $A[X]$ n'est pas principal.
\end{enumerate}
\end{ex}
...
\begin{itemize}
\item[$1$.] Soient $I$ un ensemble non vide et $(a_i)\in \Z^I\setminus\{0\}$. Montrer que $\{a_i|\ i\in I\}$ admet un plus grand diviseur commun, c'est à dire qu'il existe $d\in \N$ tel que $d$ divise $a_i$, pour tout $i\in I$ et que si $d'\in \Z$ est tel que $d'$ divise $a_i$ pour tout $i\in I$, alors $d'$ divise $d$. Montrer qu'un tel $d$ est unique.
...
\begin{ex}
\begin{enumerate}
\item Soient $a,b\in \Z\setminus\{0\}$. Montrer qu'il existe un unique  $m\in \N$  tel que : \[ \forall x\in \Z, (a\mid x\text{ et }b\mid x)\Leftrightarrow m\mid x.\] On le note $a\vee b$. 

\item Décrire $m$ en utilisant la décomposition de $a$ et $b$ en produits de facteurs premiers.

\item En déduire une relation entre $a \wedge b$ et $a\vee b$.
\end{enumerate}

\end{ex}
\begin{comment}
\textbf{Correction :} 1) Existence : Soit $x\in \Z$. Alors $a\mid x$ si  et seulement si $x\in \Z a$ et $b\mid x$ si et seulement si $x\in \Z b$. On a donc ( $x\mid a$ et $x\mid b$) si et seulement si $x\in \Z a\cap \Z b$. De plus, $\Z a \cap \Z b$ est un idéal de $\Z$, qui est principal, il existe  $m\in \Z$ tel que $\Z a\cap \Z b=m\Z$. Quitte à changer $m$ en $-m$, on peut supposer que $m\in \N$. Alors $m$ convient.

Unicité : Soit $m'\in \N$ vérifiant la condition du 1). Alors $m'$ est un multiple de $a$ et de $b$, donc par définition de $m$, $m\mid m'$. Par symétrie, $m'\mid m$, donc $m=m'$.

2) Écrivons $a=\epsilon \prod_{i=1}^k p_i^{\alpha_i}$, $b=\epsilon'\prod_{i=1}^k p_i^{\beta_i}$, où $k\in \N$,  $p_1,\ldots, p_k\in \mathbb{P}$ sont distincts, $\epsilon,\epsilon'\in \{-1,1\}$ et $\alpha_1,\ldots,\alpha_k,\beta_1,\ldots,\beta_k\in \N$. Posons $m=\prod_{i=1}^k p_i^{\gamma_i}$, où $\gamma_i=\max(\alpha_i,\beta_i)$, pour tout $i\in \llbracket 1,k\rrbracket$. Alors $m=\epsilon \left(\prod_{i\in \llbracket 1,k\rrbracket} p_i^{\gamma_i-\alpha_i}\right)a$, et $\left(\prod_{i\in \llbracket 1,k\rrbracket} p_i^{\gamma_i-\alpha_i}\right)\in \Z$ (car $\gamma_i- \alpha_i\geq 0$ pour tout $i\in \llbracket 1,k\rrbracket$) donc $m$ est un multiple de $a$, et de même, $m$ est un multiple de $b$.

 Soit $x\in \Z$ tel que $a\mid x$ et $b\mid x$.  Soit $i\in \llbracket 1,k\rrbracket$. Alors $p^{\alpha_i}\mid a$ donc $p^{\alpha_i}\mid m$  et $p^{\beta_i}\mid b$, donc $p^{\beta_i}\mid m$. Comme $\gamma_i\in \{\alpha_i,\beta_i\}$, on en déduit que $p^{\gamma_i}$ divise $x$ pour tout $i\in \llbracket 1,k\rrbracket$. Comme $p_1\neq p_2$ (par hypothèse) et que $p_1$ et $p_2$ sont premiers, on a  $p_1^{\gamma_1} \wedge p_2^{\gamma_2}=1$, par le lemme d'Euclide. D'après le lemme de Gauss, on en déduit que $p_1^{\gamma_1}p_2^{\gamma_2}$ divise $x$. Par le lemme d'Euclide, $p_1^{\gamma_1}p_2^{\gamma_2}\wedge p_3^{\gamma_3}=1$, donc par le lemme de Gauss, $p_1^{\gamma_1} p_2^{\gamma_2} p_3^{\gamma_3}$ divise $x$. Par récurrence, $m\mid x$. On en déduit que $m$ satisfait la condition du 1), donc $m=a\vee b$.

3) Comme $a\wedge b=\prod_{i\in \llbracket 1,k\rrbracket} p_i^{\min(\alpha_i,\beta_i)}$, on a $(a\vee b)(a\wedge b)=\prod_{i=1}^k p_i^{\min(\alpha_i,\beta_i)+\max(\alpha_i,\beta_i)}$. De plus, si $\alpha,\beta\in \R$, $\min(\alpha,\beta)+\max(\alpha,\beta)=\alpha+\beta$, donc $(a\vee b)(a\wedge b)=\prod_{i\in \llbracket 1,k\rrbracket} p_i^{\alpha_i+\beta_i} =|a| |b|$. 
...
\item[$3.$] Pour $n \in \mathbb{N}$, on pose $F_n=2^{2^{n}}+1$. Montrer que les nombres $\{ F_n \}_n$ sont premiers entre eux deux à deux (on pourra montrer que si $m,n\in \N^*$, avec $n>m$, il existe $a\in \Z$ tel que $F_n-a F_m=2$. On pourra commencer par le cas $n=m+1$ et $n=m+2$ pour s'inspirer). En déduire une autre  démonstration de l'infinitude des nombres premiers. 
\end{itemize}
\end{ex}
...
(3) Soient $m,n\in \N$ tels que $m\neq n$. Quitte à échanger $m$ et $n$, on peut supposer que $m<n$. On a $2^n=2^m.2^{n-m}$. On a donc $2^{2^n}=2^{2^m.2^{n-m}}=(2^{2^m})^{2^{n-m}}$ et donc \[\begin{aligned} F_n-1 &=(F_m-1)^{2^{n-m}}\\
&=\sum_{k=0}^{2^{n-m}} \binom{2^{n-m}}{k}(-1)^{2^{n-m}-k} (F_m)^{k} \\
&= \sum_{k=1}^{2^{n-m}}\binom{2^{n-m}}{k}(-1)^{2^{n-m}-k} (F_m)^{k} +\binom{2^{n-m}}{0}(-1)^{2^{n-m}-k}\\
&= \sum_{k=1}^{2^{n-m}}\binom{2^{n-m}}{k}(-1)^{2^{n-m}-k} (F_m)^{k} +1\equiv 1[F_m]
\end{aligned}.\] On en déduit que tout diviseur commun à $F_m$ et $F_n$ divise $2$, et comme $F_m$ et $F_n$ sont impairs, $F_m\wedge F_n =1$. 
