...
\textbf{Exercice~\ref{ex_divisibilite}}
...
4) $(1+\sqrt{-1})^4=(1+-1+2\sqrt{-1})^2=(2\sqrt{-1})^2=\overline{-4}=\overline{3}$. On constate que $\overline{3}$ est un élément de  $\F_7$ et que c'est une racine de $\overline{2}$.

5) Comme $\KK$  est de caractéristique $p$ (c'est à dire que $p.1_\KK=0_\KK$), on a $(\alpha+\beta)^p=\alpha^p+\beta^p$, pour tout $\alpha,\beta\in \KK$. 

6) Cf cours.

7) On a $((\sqrt{a^2-n})^p)^2=((\sqrt{a^2-n})^2)^p=(a^2-n)^p=a^2-n$, d'après la question précédente. Dans $\KK$, $a^2-n$ a exactement deux racines : $\sqrt{a^2-n}$ et $-\sqrt{a^2-n}$. Comme $\sqrt{a^2-n}\notin \F_p$, on  a $\sqrt{a^2-n}^p\neq \sqrt{a^2-n}$ donc $(\sqrt{a^2-n})^p)=-\sqrt{a^2-n}$. 

8) Soit  $b=(a+\sqrt{a^2-n})^{(p+1)/2}$. Alors \begin{align*}b^2&=(a+\sqrt{a^2-n})^{p+1}=(a+\sqrt{a^2-n})^{p}(a+\sqrt{a^2-n})\\
...
9) Par hypothèse, $n$ est un carré dans $\F_p$.   Le polynôme $Y^2-n\in \KK[Y]$ s'écrit $(Y-b)(Y+b)$. Ses deux racines sont donc $b$ et $-b$. On en déduit que $b$ est dans $\F_p$.

\textbf{Deuxième partie :}


1) On peut calculer $\left(\frac{a^2-n}{p}\right)=(a^2-n)^{(p-1)/2}$ dans $\F_p$. On a vu en cours que le coût du calcul d'une multiplication dans $\Z/p\Z$ est $O(\log(p)^2)$. Avec l'exponentiation rapide, on effectue $O(\log(p))$ calculs de produits dans $\F_p$, on a donc un coût en $O(\log(p)^3)$. 

2) Soit $(a,b)\in \F_p^2$. Si $z=0$, alors $(a,b)\in X_z$ implique $a^2=b^2$ ce qui implique que $n=0$, ce qui contredit notre hypothèse. On a donc $X_0=\emptyset$. 

On choisit maintenant $z\in \F_p^\times$. Soit $(a,b)\in \F_p^2$. Alors $(a,b)\in X_z$ si et seulement si $\begin{cases} a^2-n&=b^2\\ a+b &=z \end{cases}$ si et seulement si $\begin{cases} (a-b)(a+b)&=n\\ a+b &=z \end{cases}$ si et seulement si $\begin{cases} (a-b)&=n/z\\ a+b &=z \end{cases}$ si et seulement si $\begin{cases} (a-b)&=n/z\\ a+b &=z \end{cases}$ si et seulement si $\begin{cases} a&=1/2(z+n/z)\\  b &=1/2(z-n/z) \end{cases}$. On a donc $X_z=\{1/2(z+n/z,z-n/z)\}$. En particulier, $|X_z|=1$. On en déduit :  \[|X|=|\bigsqcup_{z\in \F_p} X_z|=|\bigsqcup_{z\in \F_p^\times} X_z|=\sum_{z\in \F_p^\times} |X_z|=(p-1).\]

3) On a $X\cap (\F_p\times \{0\})=\{(a,0)\in \F_p\times \{0\}\mid a^2-n=0\}$, donc $X\cap (\F_p\times \{0\})=\{(\pm \gamma,0)\}$, où $\gamma$ est une racine de $n$.

4) Soit $a\in \pi(E)$. Alors $\{b\in \F_p\mid (a,b)\in X\}$ est l'ensemble des racines carrées de $a^2-n$, qui est un élément non nul de $\F_p$. Cet ensemble est de cardinal $2$. 

5) On a $X\setminus E=X\cap (\F_p\times \{0\})$. Soit $a\in X\cap (\F_p\times \{0\})$. Alors $a^2-n=0$, donc $\pi^{-1}(\{a\})=\{(a,0)\}$.

6)  Commençons par déterminer $|\pi(X)$.  D'après les questions 4) et 5) $\pi(E)\cap \pi(X\setminus E)=\emptyset$. On a \[\pi(X)=\pi(E\cup X\setminus E)=\pi(E)\cup \pi(X\setminus E)=\pi(E)\sqcup \pi(X\setminus E),\]  donc $|\pi(X)|=|\pi(E)|+|\pi(X\setminus E)|$.

On a $E=\bigsqcup_{a\in \pi(E)}\pi^{-1}(\{a\})$, donc $|E|=\sum_{a\in \pi(E)}|\pi^{-1}(\{a\})|=2|\pi(E)|$, donc $|\pi(E)|=(p-3)/2$. 

On a $|X\setminus E|=|\bigcup_{a\in \pi(X\setminus E)} \pi^{-1}(\{a\})|=|X\setminus E|=2$. On en déduit que $\pi(X)=(p+1)/2$. De plus, $\pi(X)$ est l'ensemble des $a\in \F_p$ tels que $a^2-n$ est un carré, donc $|\{a\in \F_p\mid a^2-n \text{ n'est pas un carré }\}|=p-(p+1)/2=(p-1)/2$. 
